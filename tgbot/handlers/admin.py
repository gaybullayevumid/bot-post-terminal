...
async def admin_start(msg: types.Message, state: FSMContext):
    logger.info(f"[ADMIN] {msg.from_user.id} - Вход в admin_start")
    await state.clear()
    await msg.answer("👋 Привет, админ! Выберите действие:", reply_markup=KeyboardFactory.admin_menu())
    logger.info(f"[ADMIN] {msg.from_user.id} - admin_start завершён")


async def admin_get_user_count(msg: types.Message):
    logger.info(f"[ADMIN] {msg.from_user.id} - Вход в admin_get_user_count")
    admin_service = AdminService(msg.bot.db)
    users_count = await admin_service.get_users_count()
    logger.info(f"[ADMIN] {msg.from_user.id} - Всего пользователей: {users_count}")
...
async def admin_menu(call: types.CallbackQuery, state: FSMContext):
    ...
    logger.info(f"[ADMIN] {call.from_user.id} - Вход в admin_menu")
    await state.clear()
...
async def admin_invoices_start(call: types.CallbackQuery, state: FSMContext):
    ...
    logger.info(f"[ADMIN] {call.from_user.id} - Вход в admin_invoices_start")
...
async def admin_year_selected(call: types.CallbackQuery, state: FSMContext):
    ...
    logger.info(f"[ADMIN] {call.from_user.id} - Выбран год: {year}")
...
async def admin_month_selected(call: types.CallbackQuery, state: FSMContext):
    ...
    logger.info(f"[ADMIN] {call.from_user.id} - Выбран месяц: {month} для года: {year}")
    ...
    try:
        ...
        logger.info(f"[ADMIN] {call.from_user.id} - Найдено накладных: {len(filtered_invoices)}")
        
        await state.update_data(filtered_invoices=filtered_invoices, current_page=0)
        
        if not filtered_invoices:
            logger.info(f"[ADMIN] {call.from_user.id} - Нет накладных за {month}/{year}")
        ...
        logger.error(f"[ADMIN] {call.from_user.id} - Ошибка при загрузке накладных: {e}")
        await call.message.edit_text(
...
async def admin_invoice_details(call: types.CallbackQuery, state: FSMContext):
    ...
    logger.info(f"[ADMIN] {call.from_user.id} - Запрошены детали накладной: {sales_id}")
    ...
    try:
        ...
        logger.info(f"[ADMIN] {call.from_user.id} - Найдено строк в деталях: {len(details) if details else 0}")
        
        if not details:
            logger.info(f"[ADMIN] {call.from_user.id} - Детали накладной не найдены: {sales_id}")
            await call.message.edit_text(
        ...
        logger.error(f"[ADMIN] {call.from_user.id} - Ошибка при загрузке деталей накладной: {e}")
        await call.message.edit_text(
...
async def admin_back_to_invoices_list(call: types.CallbackQuery, state: FSMContext):
    ...
    logger.info(f"[ADMIN] {call.from_user.id} - Возврат к списку накладных")
    data = await state.get_data()
    ...
    if not filtered_invoices:
        logger.info(f"[ADMIN] {call.from_user.id} - Список накладных пуст")
...
async def admin_page_navigation(call: types.CallbackQuery, state: FSMContext):
    ...
    logger.info(f"[ADMIN] {call.from_user.id} - Навигация по страницам: страница {page}")
...
async def admin_stats(call: types.CallbackQuery, state: FSMContext):
    """Показать статистику"""
    logger.info(f"[ADMIN] {call.from_user.id} - Вход в admin_stats")
    await state.clear()
    ...
    try:
        ...
        logger.info(f"[ADMIN] {call.from_user.id} - Всего пользователей: {users_count}")
        ...
        logger.error(f"[ADMIN] {call.from_user.id} - Ошибка при получении статистики: {e}")
        await call.message.edit_text(
...
async def admin_download_invoice(call: types.CallbackQuery, state: FSMContext):
    ...
    logger.info(f"[ADMIN] {call.from_user.id} - Генерация Excel накладной: {sales_id}")
    ...
    try:
        ...
        if not details:
            logger.info(f"[ADMIN] {call.from_user.id} - Накладная не найдена: {sales_id}")
            await call.message.edit_text(
        ...
        logger.info(f"[ADMIN] {call.from_user.id} - Excel накладная сгенерирована: {excel_path}")
        ...
        logger.error(f"[ADMIN] {call.from_user.id} - Ошибка при генерации Excel накладной: {e}")
        await call.message.edit_text(
...
async def admin_reconciliation_menu(call: types.CallbackQuery, state: FSMContext):
    logger.info(f"[ADMIN] {call.from_user.id} - Вход в admin_reconciliation_menu")
...
async def admin_reconciliation_year(call: types.CallbackQuery, state: FSMContext):
    ...
    logger.info(f"[ADMIN] {call.from_user.id} - Выбран год для акта сверки: {year}")
...
async def admin_reconciliation_month(call: types.CallbackQuery, state: FSMContext):
    ...
    logger.info(f"[ADMIN] {call.from_user.id} - Найдено покупателей: {len(customers)}")
    if not customers:
        await call.message.edit_text(
...
async def admin_reconciliation_customer(call: types.CallbackQuery, state: FSMContext):
    ...
    logger.info(f"[ADMIN] {call.from_user.id} - Выбран покупатель: {phone} за {month}/{year}")
    admin_service = AdminService(call.bot.db)
    summary = await admin_service.get_reconciliation_data(phone, year, month)
    logger.info(f"[ADMIN] {call.from_user.id} - Найдено строк для акта сверки: {len(summary) if summary else 0}")
    if not summary:
        await call.message.edit_text(
...
async def admin_reconciliation_download_excel(call: types.CallbackQuery, state: FSMContext):
    ...
    year, month = int(year), int(month)
    logger.info(f"[ADMIN] {call.from_user.id} - Генерация Excel для {phone} за {month}/{year}")
    ...
    try:
        ...
        logger.info(f"[ADMIN] {call.from_user.id} - Excel файл сгенерирован: {file_path}")
        ...
        logger.error(f"[ADMIN] {call.from_user.id} - Ошибка при генерации Excel: {e}")
