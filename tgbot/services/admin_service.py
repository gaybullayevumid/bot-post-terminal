...
class AdminService(BaseService):
    ...
    async def get_users_count(self) -> int:
        ...
        logger.info("AdminService.get_users_count called")
        try:
            ...
            logger.info(f"AdminService.get_users_count result: {result}")
            return result
        except Exception as e:
            logger.error(f"AdminService.get_users_count error: {e}")
            raise
    
    async def get_invoices_by_period(self, year: int, month: int) -> List[Dict[str, Any]]:
        """Получить накладные за период с кэшированием"""
        logger.info(f"AdminService.get_invoices_by_period called: year={year}, month={month}")
        try:
            from tgbot.models.models import TGUser
            result = await TGUser.get_sales_invoices_by_period(self.db, year, month)
            logger.info(f"AdminService.get_invoices_by_period result count: {len(result)}")
            return result
        except Exception as e:
            logger.error(f"AdminService.get_invoices_by_period error: {e}")
            raise
    
    async def get_invoice_details(self, sales_id: int) -> Optional[List[Dict[str, Any]]]:
        """Получить детали накладной с кэшированием"""
        logger.info(f"AdminService.get_invoice_details called: sales_id={sales_id}")
        try:
            from tgbot.models.models import TGUser
            result = await TGUser.get_sales_document_details(self.db, sales_id)
            logger.info(f"AdminService.get_invoice_details result count: {len(result) if result else 0}")
            return result
        except Exception as e:
            logger.error(f"AdminService.get_invoice_details error: {e}")
            raise
    ...
    async def get_reconciliation_data(self, phone: str, year: int, month: int) -> List[Dict[str, Any]]:
        ...
        logger.info(f"AdminService.get_reconciliation_data called: phone={phone}, year={year}, month={month}")
        try:
            from tgbot.models.models import TGUser
            result = await TGUser.get_customer_sales_summary(self.db, phone, year, month)
            logger.info(f"AdminService.get_reconciliation_data result count: {len(result)}")
            return result
        except Exception as e:
            logger.error(f"AdminService.get_reconciliation_data error: {e}")
            raise
    
    async def get_sales_years(self) -> List[int]:
        """Получить список годов с продажами с кэшированием"""
        logger.info("AdminService.get_sales_years called")
        try:
            from tgbot.models.models import TGUser
            result = await TGUser.get_sales_years(self.db)
            logger.info(f"AdminService.get_sales_years result: {result}")
            return result
        except Exception as e:
            logger.error(f"AdminService.get_sales_years error: {e}")
            raise
    
    async def get_sales_months(self, year: int) -> List[int]:
        """Получить список месяцев с продажами за год с кэшированием"""
        logger.info(f"AdminService.get_sales_months called: year={year}")
        try:
            from tgbot.models.models import TGUser
            result = await TGUser.get_sales_months(self.db, year)
            logger.info(f"AdminService.get_sales_months result: {result}")
            return result
        except Exception as e:
            logger.error(f"AdminService.get_sales_months error: {e}")
            raise
    ...
    async def get_customers_by_period(self, year: int, month: int) -> List[Dict[str, Any]]:
        ...
        logger.info(f"AdminService.get_customers_by_period called: year={year}, month={month}")
        try:
            from tgbot.models.models import TGUser
            result = await TGUser.get_customers_by_period(self.db, year, month)
            logger.info(f"AdminService.get_customers_by_period result count: {len(result)}")
            return result
        except Exception as e:
            logger.error(f"AdminService.get_customers_by_period error: {e}")
            raise
        ...
        try:
            async def _get_all_customers():
                ...
                async with self.db() as session:
                    ...
                    logger.info(f"AdminService.get_all_customers_with_sales: count={len(rows)}")
                    return rows
            return await _get_all_customers()
        except Exception as e:
            logger.error(f"AdminService.get_all_customers_with_sales error: {e}")
            raise
