...
async def generate_reconciliation_act_excel(
    ...
    ws['C3'] = f"взаимных расчетов за период: {period_start} - {period_end}"
    ws['C3'].alignment = center
    ws.merge_cells('C4:F4')
    ws['C4'] = f"между: {company1} и {company2}"
    ws['C4'].alignment = center

    ws.merge_cells('B6:I6')
    ws['B6'] = f"Мы, нижеподписавшиеся, {company1} с одной стороны, и {company2}, с другой стороны, составили данный акт сверки в том, что, состояние взаимных расчетов по данным учета следующее:"
    ...
    for col in range(2, 10):
        ws.cell(row=8, column=col).font = bold
        ws.cell(row=8, column=col).alignment = center
        ws.cell(row=8, column=col).border = border

    # Сальдо начальное
    ws.merge_cells('B9:C9')
    ws['B9'] = "Сальдо начальное"
    ws['D9'] = f"{saldo_start:,.2f}"
    ws['E9'] = "0.00"
    ws['F9'] = "0.00"
    ws['G9'] = "0.00"
    ws['H9'] = "0.00"
    ...
    for op in act_data:
        ...
        ws[f'D{row}'] = f"{op['Дебет1']:,.2f}"
        ws[f'E{row}'] = f"{op['Долг1']:,.2f}"
        ws[f'F{row}'] = f"{op['Долг2']:,.2f}"
        ws[f'G{row}'] = f"{op['Дебет2']:,.2f}"
        ws[f'H{row}'] = f"{op['Кредит']:,.2f}"
        ws[f'I{row}'] = f"{op['Кредит2']:,.2f}"
        for col in range(2, 10):
    ...
    ws[f'D{row}'] = f"{sum(op['Дебет1'] for op in act_data):,.2f}"
    ws[f'E{row}'] = f"{sum(op['Долг1'] for op in act_data):,.2f}"
    ws[f'F{row}'] = f"{sum(op['Долг2'] for op in act_data):,.2f}"
    ws[f'G{row}'] = f"{sum(op['Дебет2'] for op in act_data):,.2f}"
    ws[f'H{row}'] = f"{sum(op['Кредит'] for op in act_data):,.2f}"
    ws[f'I{row}'] = f"{sum(op['Кредит2'] for op in act_data):,.2f}"
    for col in range(2, 10):
        ws.cell(row=row, column=col).alignment = center
    ...
    # Итоговая строка с суммой прописью (красный цвет, жирный)
    sum_words = num2words(abs(saldo_end), lang='ru').capitalize()
    if saldo_end < 0:
        sum_words = f"минус {sum_words}"
    ws.merge_cells(f'B{row}:I{row}')
    ws[f'B{row}'] = f"В пользу {company1 if saldo_end > 0 else company2} {abs(saldo_end):,.2f} сум ({sum_words} сум)"
...
if __name__ == "__main__":
    ...
    logger.info(f"Накладная создана: {invoice_path}")
