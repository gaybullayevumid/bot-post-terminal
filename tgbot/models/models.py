...
class TGUser(Base):
    ...
    async def add_user(cls, db_session: sessionmaker, telegram_id: int, firstname: str, lastname: str,
        ...
        INSERT INTO telegram_users (
            telegram_id, firstname, lastname, username, phone, lang_code
        ) VALUES (
            :telegram_id, :firstname, :lastname, :username, :phone, :lang_code
        );
        """
    ...
    async def get_user_by_filter(cls, db_session: sessionmaker, **kwargs):
        """
        Получить пользователя по произвольному фильтру (kwargs)
        
        SQL:
        SELECT *
        FROM telegram_users
        WHERE <фильтр из kwargs>;
        """
        async with db_session() as session:
...
# =============================== CUSTOMER ===============================
    ...
    @classmethod
    async def get_customer_sales_summary(cls, db_session: sessionmaker, phone_number: str = None, year: int = None, month: int = None, customer_id: int = None):
        """
        Получить сводку продаж по клиенту (по номеру телефона, с фильтрами по году/месяцу)
        
        SQL:
        SELECT
            s.sls_datetime AS `Дата`,
            CONCAT('Реализация №', s.sls_id) AS `Документ`,
            -- Итоговая сумма продажи из первого подзапроса
            COALESCE(sale_totals.total_amount, 0) AS `Сумма`,
            -- Итоговая сумма оплаты из второго подзапроса (теперь с правильными полями)
            COALESCE(paid_totals.paid_amount, 0) AS `Оплачено`,
            -- Долг рассчитывается как разница
            (COALESCE(sale_totals.total_amount, 0) - COALESCE(paid_totals.paid_amount, 0)) AS `Долг`,
            s.sls_note AS `Примечание`,
            c.cstm_name AS `Имя покупателя`,
            c.cstm_phone2 AS `Телефон покупателя`,
            c.cstm_id AS `ID Покупателя`
        FROM
            doc_sales AS s
        LEFT JOIN (
            -- ПОДЗАПРОС 1: Считаем полную сумму каждой продажи (здесь всё было верно)
            SELECT
                op.opr_document AS sale_id,
                SUM(op.opr_quantity * a.oap_price1) AS total_amount
            FROM
                operations AS op
            JOIN
                operations_additional_prop AS a ON a.oap_operation = op.opr_id
            WHERE
                op.opr_type = 2
            GROUP BY
                op.opr_document
        ) AS sale_totals ON s.sls_id = sale_totals.sale_id
        LEFT JOIN (
            -- ПОДЗАПРОС 2: Считаем сумму оплат, используя правильную таблицу и столбцы
            SELECT
                pmt_document AS sale_id,        -- <<< ИСПРАВЛЕНО: Связь через pmt_document
                SUM(pmt_value) AS paid_amount   -- <<< ИСПРАВЛЕНО: Сумма из pmt_value
            FROM
                doc_payments
            GROUP BY
                pmt_document
        ) AS paid_totals ON s.sls_id = paid_totals.sale_id
        LEFT JOIN -- Присоединяем покупателей
            dir_customers AS c ON s.sls_customer = c.cstm_id
        WHERE
            -- Укажите нужный фильтр по покупателю
            YEAR(s.sls_datetime) = :year              -- ФИЛЬТР ПО ГОДУ (замените на нужное значение, например, 2024)
            AND MONTH(s.sls_datetime) = :month        -- ФИЛЬТР ПО МЕСЯЦУ (замените на нужное значение, например, 7)
            -- Выберите ОДИН из следующих фильтров для покупателя (раскомментируйте нужный и заполните значение):
            AND c.cstm_id = :cutomer                  -- ФИЛЬТР ПО ID ПОКУПАТЕЛЯ (замените на ID, например, 123)
            AND (c.cstm_phone = :phone OR             -- ФИЛЬТР ПО НОМЕРУ ТЕЛЕФОНА (замените на номер в кавычках, например, '998901234567')
                c.cstm_phone2 = :phone OR
                c.cstm_phone3 = :phone OR
                c.cstm_phone4 = :phone)
            AND s.sls_performed = 1
            AND s.sls_deleted = 0
        ORDER BY
...
# =============================== YEARS ===============================
    ...
    async def get_sales_years(cls, db_session: sessionmaker):
        ...
        SQL:
        SELECT DISTINCT
            EXTRACT(YEAR FROM sls_datetime) AS year
        FROM doc_sales
        WHERE
            sls_performed = 1
            AND sls_deleted = 0
...
# =============================== MONTHS ===============================
    ...
    async def get_sales_months(cls, db_session: sessionmaker, year: int):
        ...
        SQL:
        SELECT DISTINCT
            EXTRACT(MONTH FROM sls_datetime) AS month
        FROM doc_sales
        WHERE
            sls_performed = 1
            AND sls_deleted = 0
            AND EXTRACT(YEAR FROM sls_datetime) = :year
        ORDER BY month;
        """
        async with db_session() as session:
...
# =============================== CUSTOMERS ===============================
    ...
    async def get_customers_by_period(cls, db_session: sessionmaker, year: int, month: int = None):
        ...
        WHERE
            ...
            [AND EXTRACT(MONTH FROM s.sls_datetime) = :month]
        GROUP BY c.cstm_id, c.cstm_name, c.cstm_phone
...
# =============================== CUSTOMER NAME ===============================
    ...
    async def get_customer_name_by_phone(cls, db_session: sessionmaker, phone_number: str):
        """
        Получить имя клиента по номеру телефона
        ...
        WHERE
            c.cstm_phone = :phone_number
            OR c.cstm_phone2 = :phone_number
            OR c.cstm_phone3 = :phone_number
            OR c.cstm_phone4 = :phone_number
        LIMIT 1;
        """
        async with db_session() as session:
            stmt = select(
...
# =============================== MONTHLY SALES SUMMARY ===============================
    ...
    @classmethod
    async def get_monthly_sales_summary(cls, db_session: sessionmaker, year: int, month: int):
        """
        Получить сводку продаж за месяц
        ...
        async with db_session() as session:
            ...
            stmt = select(
                ...
                text("YEAR(s.sls_datetime) = :year"),
                text("MONTH(s.sls_datetime) = :month"),
                text("s.sls_performed = 1"),
                text("s.sls_deleted = 0")
            ).group_by(
                text("s.sls_id"),
            ...
            result = await session.execute(stmt, {"year": year, "month": month})
            rows = result.fetchall()
            return [row._asdict() for row in rows]
...
# =============================== FILTERED SALES SUMMARY ===============================
    ...
    @classmethod
    async def get_filtered_sales_summary(
        cls,
        ...
        """
        Получить сводку продаж с фильтрацией по клиенту или телефону
        ...
        WHERE
            ...
            [AND (c.cstm_phone = :phone_number OR ...)]
        GROUP BY s.sls_id, s.sls_datetime
        ORDER BY s.sls_datetime DESC, s.sls_id DESC;
        """
        async with db_session() as session:
            ...
            if customer_id is not None:
                conditions.append(text("c.cstm_id = :customer_id"))
                ...
                params["phone_number"] = phone_number
            stmt = stmt.where(*conditions).group_by(
                text("s.sls_id"),
                text("s.sls_datetime")
            ).order_by(
                text("s.sls_datetime DESC"),
